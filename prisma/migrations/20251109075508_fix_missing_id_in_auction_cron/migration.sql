...
BEGIN
    ...
    LOOP
        ...
        IF FOUND THEN
            ...
            SELECT gen_random_uuid(), w.id, ad.amount, 'AUCTION_DEPOSIT_REFUND', 'COMPLETED', 'INTERNAL', 'Refund for losing auction on ' || listing_type_text || ' #' || auction_record.id, now(), now()
            FROM "AuctionDeposit" ad
